    ...
    self.output = function(widget, options) {
      ...
      _.each(widget._permalinkDocs || [], function(doc) {
        ...
        while (true) {
          ...
          var shouldUpdateTitle = _.inRange(content.indexOf('updateTitle=1', i), href, close);
          var shouldAddDownloadAttr = _.inRange(content.indexOf('download=1', i), href, close);
          ...
          if (shouldAddDownloadAttr) {
            right = right || content.indexOf('>', left);
            if ((right !== -1)) {

              var downloadName = self.apos.utils.escapeHtml(doc.title);
              if (doc.attachment && doc.attachment.extension) {
                downloadName += '.' + doc.attachment.extension;
              }

              content = content.substr(0, right) + ' download="' + downloadName + '"' + content.substr(right);
            }
          }
        }
      });
